...
def plot_arr(arr, save=None):
    labels =["VOUT", "VX","VOUT_SAMPL","VX_SAMPL"]
    x = arr[:, 0]
    ...
    plt.legend()
    plt.tight_layout()
    plt.xlim(0.000036699,0.000036707)
    plt.ylim(1.795,1.82)
    out = save or "comparator_sn.png"
    plt.savefig(out, dpi=200)
    print(out)

def main():
    p = argparse.ArgumentParser(description="Plot ngspice wrdata output (no header, two-space sep)")
    p.add_argument("file", nargs="?", default="sim/LELO_BOOST_STEADY_STATE_TURN/output_tran/comparator_data.txt")
