...
def plot_arr(arr, save=None):
    ...
    for c in range(arr.shape[1]):
        if c % 2 == 0:
            continue
        plt.plot(x, arr[:, c], label=labels[c//2] if c//2 < len(labels) else f"col {c}")
    plt.xlabel("Time [s]")
    plt.ylabel("Voltage [V]")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    out = save or "30mV_startup_fail_lss.png"
    plt.savefig(out, dpi=200)
    print(out)

def main():
    p = argparse.ArgumentParser(description="Plot ngspice wrdata output (no header, two-space sep)")
    p.add_argument("file", nargs="?", default="sim/LELO_EXSC/output_tran/output.txt")
